...
import com.alinaj.dev.openconnect.core.UserDialog;
...
public class AuthFormHandler extends UserDialog
	...
	private void saveAndStore() {
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			switch (opt.type) {
			case LibOpenConnect.OC_FORM_OPT_TEXT: {
				opt.value = ConfigUtil.getInstance(mContext).getUsername();
				break;
			}
			case LibOpenConnect.OC_FORM_OPT_PASSWORD: {
				String s = ConfigUtil.getInstance(mContext).getPassword();
				opt.value = s;
				break;
			}
			case LibOpenConnect.OC_FORM_OPT_SELECT:
	...
	public Object earlyReturn() {
		...
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			switch (opt.type) {
			case LibOpenConnect.OC_FORM_OPT_PASSWORD:
				String password = ConfigUtil.getInstance(mContext).getPassword();
				if (password.isEmpty()) {
					return null;
				}
				opt.value = password;
				break;
			case LibOpenConnect.OC_FORM_OPT_TEXT:
				String username = ConfigUtil.getInstance(mContext).getUsername();
				if (username.isEmpty()) {
					return null;
				}
				opt.value = username;
