...
import com.alinaj.dev.openconnect.core.OpenVpnService.LocalBinder;
...
public abstract class VPNConnector {
	...
	public abstract void showReconnectCount(String state);
	...
	private final ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder serviceBinder) {
			...
			service.updateActivityRefcount(mIsActivity ? 1 : 0);
			service.setReconnectListener(new OpenVpnService.ReconnectListener() {
				@Override
				public void showReconnectStaus(String state) {
					showReconnectCount(state);
				}
			});
			onUpdate(service);
		}
