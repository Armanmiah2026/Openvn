...
import com.alinaj.dev.connectivity.*;
...
public class OpenVPNService extends VpnService implements VPNUtil.VPNProtectListener, Callback, net.openvpn.openvpn.OpenVPNClientThread.EventReceiver {
        ...
        public Profile(String location_arg, String filename, boolean filename_is_url_encoded_profile_name, ClientAPI_EvalConfig ec) {
            ...
            if (filename_is_url_encoded_profile_name) {
                ...
                if (VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    this.name = URLDecoder.decode(this.name, StandardCharsets.UTF_8);
                }
            } else {
                this.name = filename;
            }
    ...
    private static class ProfileFN {
        ...
        public static boolean has_ovpn_ext(String fn) {
            ...
            return fn.endsWith(".ovpn") || fn.endsWith(".OVPN");
        }
        ...
        public static String encode_profile_fn(String name) {
            if (VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                return URLEncoder.encode(name, StandardCharsets.UTF_8) + ".ovpn";
            }
            return name;
        }
    }
    ...
    private class TunBuilder extends VpnService.Builder implements net.openvpn.openvpn.OpenVPNClientThread.TunBuilder {
        ...
        public boolean tun_builder_add_address(String address, int prefix_length, String gateway, boolean ipv6, boolean net30) {
            try {
                Log.d(OpenVPNService.TAG, String.format("BUILDER: add_address %s/%d %s ipv6=%b net30=%b", address, Integer.valueOf(prefix_length), gateway, Boolean.valueOf(ipv6), Boolean.valueOf(net30)));
        ...
        public boolean tun_builder_set_session_name(String name) {
            try {
                ...
                ConfigUtil cu = new ConfigUtil(getApplicationContext());
                if(cu.isUDP()){
    ...
    private boolean do_connect_action(String prefix, Intent intent, boolean proxy_retry) {
        ...
        String port = config.getSSHPortString();
        try {
            ...
            switch (config.getTunnelType()) {
                ...
                case ConfigUtil.MODE_SSL_HTTP_PROXY:
                    port = String.valueOf(config.getSSLPort());
                    break;
            }
            profile_content = profile_content.replace(getPort(profile_content), port);
            String remote = getIpOrHost(profile_content);
            String query_remote = "";
            profile_content = profile_content.replace(remote, config.getSSHHost());
            if (config.isQueryMode()) {
                /*if (config.getFrontQueryString().isEmpty()) {
                 query_remote = remote + "@" + config.getBackQueryString();
                 } else {
                 query_remote = config.getFrontQueryString() + "@" + remote;
                 }*/
                profile_content = profile_content.replace(remote, config.getFrontQueryString());
                profile_content = profile_content.replace(getPort(profile_content), "80");
            }
            if (config.getCustomSSLPortEnabled()) {
                profile_content = profile_content.replace(remote, config.getProxy());
                profile_content = profile_content.replace(getPort(profile_content), ""+config.getSSLPort());
            }
            String str = TAG;
            Object[] objArr = new Object[MSG_EVENT];
           ...
           if (config.getTunnelType() == ConfigUtil.MODE_OVPN_DIRECT_UDP && getPackageName().contains(XMLRPC.TAG)) {
               ...
               profile_content += "proto udp\n";
            } else {
               profile_content += "proto tcp\n";
           }
    ...
    private String getIpOrHost(String config)
    {
        ...
        for (String line: split) {
            if (line.toLowerCase().contains("remote ")) {
                ...
                String mPosibleIp = split_line[1];
                return mPosibleIp;
            }
        }
        return "None";
    ...
    private String getPort(String config)
    {
        ...
        for (String line: split) {
            if (line.toLowerCase().contains("remote ")) {
                String[] split_line = line.split(" ");
                String mPosibleIp = split_line[2];
                return mPosibleIp;
            }
        }
        return "None";
    ...
    private void start_notification() {
        if (this.mNotifyBuilder == null && this.current_profile != null) {
            ...
            this.mNotifyBuilder.setContentIntent(get_configure_intent(MSG_EVENT)).setSmallIcon(R.drawable.ic_launcher).setContentTitle(getString(R.string.app)).setContentText(resString(R.string.notification_initial_content)).setOnlyAlertOnce(true).setOngoing(true).setWhen(new Date().getTime());
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                ...
                startForeground(NOTIFICATION_ID, this.mNotifyBuilder.getNotification(), FOREGROUND_SERVICE_TYPE_SYSTEM_EXEMPTED);
            }
        }
    }
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_EVENT /*1*/:
                ...
                if (evm.res_id == R.string.auth_failed) { /*2131034138*/
                    if (this.current_profile != null) {
                        this.current_profile.get_name();
                    }
                } else if (evm.res_id == R.string.connected) { /*2131034168*/
                    Toast.makeText(getApplicationContext(), "Connected Successfully\uD83D\uDE0D", Toast.LENGTH_SHORT).show();
                    if (this.current_profile != null) {
                        this.current_profile.reset_proxy_context();
                    }
                } else if (evm.res_id == R.string.core_thread_inactive) { /*2131034174*/
                    if (this.cpu_usage != null) {
                        this.cpu_usage.stop();
                    }
                    stop_notification();
                ...
                } else if (evm.res_id == R.string.disconnected) { /*2131034189*/
                    if (lastev != null) {
                ...
                } else if (evm.res_id == R.string.pause) {
                    paused = true;
                } else if (evm.res_id == R.string.dynamic_challenge) { /*2131034191*/
                    if (this.current_profile != null) {
                ...
                } else if (evm.res_id == R.string.pem_password_fail) { /*2131034270*/
                    evm.info = BuildConfig.FLAVOR;
                    if (this.current_profile != null) {
                        this.current_profile.get_name();
                    }
                } else if (evm.res_id == R.string.proxy_need_creds) { /*2131034322*//*if (this.current_profile != null) {
                ...
                if (this.enable_notifications) {
                    if (evm.priority == MSG_LOG) {
                        Toast.makeText(this, evm.res_id, android.widget.Toast.LENGTH_LONG).show();
