...
import com.alinaj.dev.core.CIDRIP;
import com.alinaj.dev.core.NetworkSpace;
...
public class Tunnel
{
	...
	private final AtomicReference<ParcelFileDescriptor> mTunFd;
	private final AtomicBoolean mRoutingThroughTunnel;
			...
			boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering, boolean useBypass) throws Exception
	{
		...
		try
		{
			...
			mRoutes.addIP(new CIDRIP("169.254.1.0", 24), false);

			ConfigUtil cu = new ConfigUtil(mHostService.getContext());
			mRoutes.addIP(new CIDRIP(cu.getSSHHost(), 32), false);
			// Add Dns
			if(forwardDns){
			...
			for (NetworkSpace.ipAddress route : mRoutes.getPositiveIPList()) {
				try {
					if (multicastRange.containsNet(route))
						SkStatus.logDebug("VPN: Ignoring multicast route: " + route);
					else
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			//dnsgwRelay = String.format("%s:%d", "127.0.0.1", 8091);
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
				@Override
				public void onStart(){
					mHostService.onDiagnosticMessage("DNS Forwarding Started");
				}
				@Override
				public void onStop(){
					mHostService.onDiagnosticMessage("DNS Forwarding Stopped");
					stop();
				}
			});
	...
	//	udpResolver = String.format("%s:%d", "127.0.0.1", 8091);
		...
		//startTun2Socks(tunFd,mMtu,mPrivateAddress.mRouter,VPN_INTERFACE_NETMASK,null,socksServerAddress,udpResolver,dnsgwRelay,transparentDns,true );
