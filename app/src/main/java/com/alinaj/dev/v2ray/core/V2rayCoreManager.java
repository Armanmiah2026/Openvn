package com.alinaj.dev.v2ray.core;

import static android.content.pm.ServiceInfo.FOREGROUND_SERVICE_TYPE_SYSTEM_EXEMPTED;
import static com.alinaj.dev.v2ray.utils.Utilities.getUserAssetsPath;
...
import com.alinaj.dev.v2ray.util.PluginUtil;
...
public final class V2rayCoreManager {
    ...
    private void makeDurationTimer(final Context context, final boolean enable_traffic_statics) {
        countDownTimer = new CountDownTimer(7200, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                Intent connection_info_intent = new Intent(context.getPackageName() + ".V2RAY_CONNECTION_INFO");
                connection_info_intent.putExtra("STATE", V2rayCoreManager.getInstance().V2RAY_STATE);
                connection_info_intent.putExtra("DELAY", V2RAY_DELAY);
    ...
    private void sendGraphBroadcast(Context context) {
        try {
            List<Long> findData = TrafficUtils.getData();
            Intent intent = new Intent(context.getPackageName() + ".GRAPH");
    ...
    public void setUpListener(Service targetService) {
        try {
            ...
            //Libv2ray.initV2Env(getUserAssetsPath(targetService.getApplicationContext()), V2rayConfigUtil.INSTANCE.getDeviceIdForXUDPBaseKey());
    ...
    private String getDeviceIdForXUDPBaseKey() {
        byte[] androidId = Settings.Secure.ANDROID_ID.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        return Base64.encodeToString(java.util.Arrays.copyOf(androidId, 32), Base64.NO_PADDING | Base64.URL_SAFE);
    }
    ...
    public boolean startCore(final V2rayConfig v2rayConfig) {
        ...
        profileItem = MmkvManager.INSTANCE.decodeServerConfig(MmkvManager.INSTANCE.getSelectServer());
        ...
        try {
            ...
            if (isV2rayCoreRunning()) {
                ...
                PluginUtil.INSTANCE.runPlugin(v2rayServicesListener.getService(), profileItem, v2rayConfig.CONNECTED_V2RAY_SERVER_ADDRESS + ":" + v2rayConfig.CONNECTED_V2RAY_SERVER_PORT);
            }
        } catch (Exception e) {
    ...
    private void sendDisconnectedBroadCast() {
        ...
        downloadSpeed = 0;
        if (v2rayServicesListener != null) {
            Intent connection_info_intent = new Intent(v2rayServicesListener.getService().getApplicationContext().getPackageName() + ".V2RAY_CONNECTION_INFO");
            connection_info_intent.putExtra("STATE", V2rayCoreManager.getInstance().V2RAY_STATE);
            connection_info_intent.putExtra("DURATION", SERVICE_DURATION);
...
//    }
    ...
    private String createNotificationChannelID(final String Application_name) {
        String notification_channel_id = "v2ray";
    ...
    private void showNotification(final V2rayConfig v2rayConfig) {
        ...
        mBuilder = new NotificationCompat.Builder(v2rayServicesListener.getService(), notificationChannelID);
        mBuilder.setSmallIcon(v2rayConfig.APPLICATION_ICON)
                .setContentTitle("Connected to " + name)
                .setContentText(v2rayServicesListener.getService().getString(R.string.connecting))
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
            ...
            v2rayServicesListener.getService().startForeground(1, mBuilder.build(), FOREGROUND_SERVICE_TYPE_SYSTEM_EXEMPTED);
        }
    }

    public void connected(long delay) {
        V2RAY_DELAY = delay;
        V2RAY_STATE = AppConfigs.V2RAY_STATES.V2RAY_CONNECTED;
        if (mBuilder != null && mNotificationManager != null) {
            mBuilder.setContentText(v2rayServicesListener.getService().getString(R.string.connected));
    ...
    public Long getConnectedV2rayServerDelay() {
        try {
            return v2RayPoint.measureDelay("https://www.gstatic.com/generate_204");
        } catch (Exception e) {
            return -1L;
        }
    }
