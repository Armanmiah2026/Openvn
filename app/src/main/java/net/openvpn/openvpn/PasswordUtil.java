...
public class PasswordUtil {
    ...
    private String en(String pwd_cleartext) {
        ...
        if (!(pwd_cleartext == null || this.secret == null || this.prefPrefix == null)) {
            try {
                ...
                return Base64.encodeToString(cipher.doFinal(pwd_cleartext.getBytes(StandardCharsets.UTF_8)), 2);
            } catch (Exception e) {
                Log.e(TAG, "en", e);
                regenerate(true);
            }
    ...
    private String key_string(String type, String account) {
        return String.format("%s.%s.%s", this.prefPrefix, type, account);
    }

    private String key_prefix(String type) {
        if (type != null) {
            return String.format("%s.%s.", this.prefPrefix, type);
        }
        return String.format("%s.", this.prefPrefix);
    }
